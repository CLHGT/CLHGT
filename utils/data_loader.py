...
class data_loader:
    ...
    def get_meta_struct(self, struct=[], acm=False, group=[0, 1]):
        """
        Get meta path matrix
            meta is a list of edge types (also can be denoted by a pair of node types)
            return a sparse matrix with shape [node_num, node_num]
        ...
        for x in struct:
            if acm and x == 0:
                ini = ini.dot(sum([self.links['data'][y] for y in group]))
            elif x >= len(self.links['data']):
                ini = ini.dot(sum([self.links['data'][x] for x in group]))
            else:
